...
public class BukuKontak {
    ...
    // Format baris: nama;nomor;email  (email boleh kosong)
    public void muatDariBerkas() {
        daftar.clear();
        try (BufferedReader pembaca = new BufferedReader(new FileReader(namaBerkas))) {
            String baris;
            while ((baris = pembaca.readLine()) != null) {
                String[] bagian = baris.split(";", -1); // -1 agar email kosong tetap tertangkap
                if (bagian.length >= 2) {
                    String email = bagian.length >= 3 ? bagian[2] : "";
                    daftar.add(new Kontak(bagian[0], bagian[1], email));
                }
            }
    ...
    public void hapusKontak(String nama) {
        ...
        // Simpan perubahan ke berkas secara otomatis
        simpanKeBerkas();
    }
}
